...
namespace Umbrella.Controllers
{
    ...
    public class HomeController : Controller
    {
        ...
        {
            ViewBag.ListaVariables = AE_VariablesREPO.GetAllRecords().ToList();
        ...
        public JsonResult BalancearFondo(string pass)
        {
            if (pass != "10563")
            {
                return Json(new
        ...
        public bool CerrarFondo(string pass)
        {
            if (pass != "10563")
            {
                return false;
        ...
        public JsonResult GuardarVariables(List<int> id, List<decimal> valor)
        {
            List<InstaTransfer.DataAccess.AE_Variable> variables = AE_VariableREPO.GetAllRecords().ToList();
            int i = 0;
            foreach (var item in id)
            {
                InstaTransfer.DataAccess.AE_Variable Item = variables.Where(u => u.Id == item).FirstOrDefault();
                Item.Monto = valor[i];
                AE_VariableREPO.SaveChanges();
                i++;
            }
        ...
        public JsonResult GuardarVariablesBanco(List<int> id, List<string> descripcion)
        {
            List<InstaTransfer.DataAccess.AE_Variable> variables = AE_VariableREPO.GetAllRecords().ToList();
            int i = 0;
            foreach (var item in id)
        ...
        public ActionResult DashboardFondo()
        {
            ...
            List<AE_ValorAccionTR> ValorAcctionTR = AE_ValorAccionTRREPO.GetAllRecords().ToList();
            decimal ValorGenesis = 0;
            if (ValorAcctionTR.FirstOrDefault().ValorAccion > 0)
            {
                ValorGenesis = ValorAcctionTR.FirstOrDefault().ValorAccion;
            }
            else
            {
                if (ValorAcctionTR.Count > 1)
                    ValorGenesis = ValorAcctionTR.Skip(1).FirstOrDefault().ValorAccion;
            }
            ViewBag.ValorGenesis = ValorGenesis;
            ViewBag.Acciones = acciones;

...
namespace Umbrella.Controllers
{
    ...
    public class ResumenController : Controller
    {
        ...
        URepository<Commerce> ComerceREPO = new URepository<Commerce>();
        ...
        public ActionResult Configuracion(string _fecha)
        {
            ...
            ViewBag.ValorAccionTR = AE_ValorAccionTRREPO.GetAllRecords().Take(30).OrderByDescending(u => u.FechaOperacion).ToList();
            ...
            {
                cobrado = cobrado + item.AE_EstadoCuentas.Where(u => !u.Abono && u.FechaOperacion < fecha).Sum(u => u.Monto);
        ...
        public ActionResult Dashboard()
        {
            ...
            ViewBag.Reembolso = reembolso;
            ViewBag.Prestado = prestado;
            ViewBag.PendientePorCobrar = reembolso - cobrado;
        ...
        public ActionResult Fondo()
        {
            ...
            var ValorAccion = AE_ValorAccionREPO.GetAllRecords().OrderByDescending(u => u.FechaOperacion).ToList();
            var ValorAccionTR = AE_ValorAccionTRREPO.GetAllRecords().OrderByDescending(u => u.FechaOperacion).ToList();
            var BalanceDiario = AE_BalanceDiarioREPO.GetAllRecords().OrderByDescending(u => u.FechaOperaicon).ToList();
            ...
            int skip = 30;
            if (ValorAccionTR.Count == 30 || ValorAccionTR.Count < 30)
                skip = ValorAccionTR.Count - 1;

            var lesta = AE_ValorAccionTRREPO.GetAllRecords().Where(u => u.FechaOperacion.Month == DateTime.Now.Month && u.FechaOperacion.Year == DateTime.Now.Year).OrderBy(u => u.FechaOperacion).ToList();
            DateTime busca;
            decimal rendimientomes = 0;
            if (lesta.Count() > 0)
            {
                rendimientomes = ((decimal.Parse(ValorAccionTR.FirstOrDefault().ValorAccion.ToString()) - decimal.Parse(lesta.FirstOrDefault().ValorAccion.ToString())) * 100 / decimal.Parse(lesta.FirstOrDefault().ValorAccion.ToString()));
            }
            ...
            //decimal rendimientomes = ((decimal.Parse(ValorAccionTR.FirstOrDefault().ValorAccion.ToString()) - decimal.Parse(ValorAccionTR.Skip(skip).FirstOrDefault().ValorAccion.ToString())) * 100 / decimal.Parse(ValorAccionTR.FirstOrDefault().ValorAccion.ToString()));
            ViewBag.RendimientoTotal = rendimientototal;
            ViewBag.RendimientoMes = rendimientomes;
            ...
            List<MovimientosCapital> MovimientosCapital = new List<ResumenController.MovimientosCapital>();
            List<MovimientosCapital> Cobro = new List<ResumenController.MovimientosCapital>();
            int i = 0;
            int j = 1;
            int p = 0;

            foreach (var valorn in AE_ValorAccionTRREPO.GetAllRecords().OrderByDescending(u => u.FechaOperacion).Take(30).OrderBy(u => u.FechaOperacion).ToList())
            {

                MovimientosCapital _item = new ResumenController.MovimientosCapital();
                _item.a = valorn.TotalCobroDiario;
                _item.b = valorn.CapitalPorCobrar;
                _item.y = valorn.FechaOperacion.ToString("dd MMM");

                Cobro.Add(_item);
                Accion.Add(valorn.ValorAccion);
                _fechas.Add(valorn.FechaOperacion.ToString("dd MMM"));
                Capital.Add(valorn.NuevoCapital);


            }
            foreach (var item in BalanceDiario.OrderBy(u => u.FechaOperaicon).ToList())
            {
                bool win = false;
                if (i == 5 || i == 0 || j == BalanceDiario.Count())
                {
                    win = true;
                    i = 1;
                }
                ...
                if (win)
                {
                    //Capital.Add(item.TotalCapital);
                    //_fechas.Add(item.FechaOperaicon.ToString("dd MMM"));
                    MovimientosCapital ele = new ResumenController.MovimientosCapital();
                    ele.y = item.FechaOperaicon.ToString("dd MMM");
                    ele.a = Math.Round(item.TotalPorCobrarBs / item.TasaUtilizada, 2);
                    ele.b = item.TotalCuentaUsd;
                    //ele.c = item.TotalCapital;
                    MovimientosCapital.Add(ele);
                }
                j++;
            }
            ...
            //{
            //    bool win = false;
            //    if (i == 5 || i == 0 || j == ValorAccion.Count())
            //    {
            //        win = true;
            //        i = 1;
            //    }
            ...
            //        //_fechas.Add(item.FechaOperacion.ToString("dd MMM"));
            //    }
            //    j++;
            ...
            var json2 = new JavaScriptSerializer().Serialize(MovimientosCapital);
            ViewBag.MovimientosCapital = json2;
            //Cobro = Cobro.Distinct().ToList().OrderBy(u => u.y).ToList();
            var json3 = new JavaScriptSerializer().Serialize(Cobro);
            ViewBag.MovimientosCobro = json3;

            List<Commerce> listaComercio = ComerceREPO.GetAllRecords().Where(u => u.AE_Avances.Count > 0).ToList();
            List<ComercioOrden> ListaOrden = new List<ComercioOrden>();
            foreach (var item in listaComercio)
            {
                ComercioOrden come = new ComercioOrden();
                come.rif = item.Rif;
                come.cantidad = item.AE_Avances.ToList().Count();
                come.monto = item.AE_Avances.Where(u => u.AvanceBs != null).Sum(u => u.Avance);
                ListaOrden.Add(come);
            }
            List<Commerce> listaComercioDef = new List<Commerce>();
            foreach (var item in ListaOrden.OrderByDescending(u => u.monto))
            {
                Commerce elementoinuevo = ComerceREPO.GetAllRecords().Where(u => u.Rif == item.rif).FirstOrDefault();
                listaComercioDef.Add(elementoinuevo);
            }
            ViewBag.Comercios = listaComercioDef.Take(10).ToList();
            ViewBag.ComerciosOrden = ListaOrden;
            List<AE_Avance> ListaAvance = AE_AvanceREPO.GetAllRecords().ToList();
            ViewBag.AvancesEjecutados = ListaAvance.Where(u => u.IdEstatus == 2).Count();
            ViewBag.AvancesActivos = ListaAvance.Where(u => u.IdEstatus == 1).Count();
            ViewBag.AvancesTotal = ListaAvance.Where(u => u.IdEstatus == 1 || u.IdEstatus == 2).Count();

            ViewBag.AvancesEstimado = "74";
            return View();
        }

        public class ComercioOrden
        {
        ...
        public class MovimientosCapital
        {

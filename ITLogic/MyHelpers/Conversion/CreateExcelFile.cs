...
namespace MyHelpers.ExportToExcel
{
    //
    //  November 2013
...
#region HELPER_FUNCTIONS
        ...
        /// <param name="excelFilename">Name of file to be written.</param>
        /// <returns>True if successful, false if something went wrong.</returns>
        public static bool JaniotCreateExcelDocument(DataSet ds, string excelFilename)
        {
            try
            {
                using (SpreadsheetDocument document = SpreadsheetDocument.Create(excelFilename, SpreadsheetDocumentType.Workbook))
                ...
                SystemLog.Janiot.Write("Successfully created: " + excelFilename, EventLogEntryType.Information);
                return true;
            }
            catch (Exception ex)
            {
                SystemLog.Janiot.Write("Failed, exception thrown: " + ex.Message, EventLogEntryType.Error);
                return false;
            }
        }
